    ...
    try: 
        with open(RECORD_FILE, "r", encoding="utf-8") as f:
            for line in f: 
                ...
                if len(parts) !=3:
                    continue
                name, timestamp, score_str = parts
                try:
                    score = int(score_str)
                except ValueError:
                    continue
...
def _show_leaderboard(top_n: int = 10) -> None:
    ...
    if not records:
        print ("\n You are the first player! There are no records set yet.")
        return
    ...
    print ("-" * 40)
    print (f"{'Rank': <5} {'Name': <20} {'Time (s)': >8}")
    print ("-" *40)

    for i, rec in enumerate (records [:top_n], start=1):
        print (f"{i:<5} {rec['name']: <20} {rec['score']: >8}")
...
if __name__ == "__main__":
    ...
    if DEBUG:
        ...
        placeholder_score = 999
        _record_result_and_show(name, placeholder_score)
        sys.exit(0)
    ...
    while remaining_time > 0: 
        try: 
            ...
            if current_room == "hallway" and find_item_by_name ("house keys", inventory):
                ...
                used_seconds = int(START_TIME - remaining_time)
                player_name = input("\n Enter your name for the leaderboard, please: ").strip()
                _record_result_and_show(player_name, used_seconds)
                sys.exit(0)
